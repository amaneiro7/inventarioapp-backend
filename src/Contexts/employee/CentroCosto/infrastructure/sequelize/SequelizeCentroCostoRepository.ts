...
export class SequelizeCentroCostoRepository implements CentroCostoRepository {
  ...
  async searchById(id: Primitives<CodCentroCosto>): Promise<Nullable<CentroCostoPrimitives>> {
    return await CentroCostoModel.findByPk(id) ?? null
  }

  async searchByName(name: Primitives<CentroCostoName>): Promise<Nullable<CentroCostoPrimitives>> {
