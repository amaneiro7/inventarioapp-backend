...
export class DepartamentoCreator {
  ...
  async run({ name, vicepresidenciaEjecutivaId, centroCostoId, cargos }: Omit<DepartamentoPrimitives, 'id'>): Promise<void> {
    const arraySinDuplicados = Array.from(new Set(cargos))

    await this.createDepartamentoUseCase.execute({ name, vicepresidenciaEjecutivaId, centroCostoId, cargos: arraySinDuplicados })

    const departamento = Departamento.create({ name, vicepresidenciaEjecutivaId, centroCostoId, cargos: arraySinDuplicados })
