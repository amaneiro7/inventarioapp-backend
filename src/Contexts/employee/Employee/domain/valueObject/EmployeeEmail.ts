...
export class EmployeeEmail extends StringValueObject {
  ...
  static async updateEmailField({ repository, email, entity }: { repository: EmployeeRepository, email?: Primitives<EmployeeEmail>, entity: RegularEmployee }): Promise<void> {
  ...
  static async ensureEmailDoesNotExit({ repository, email }: { repository: EmployeeRepository, email: Primitives<EmployeeEmail> }): Promise<void> {
    // Searches for a device with the given Email in the database
    const userWithEmail: Nullable<EmployeePrimitives> = await repository.searchByEmail(new EmployeeEmail(email).value)
