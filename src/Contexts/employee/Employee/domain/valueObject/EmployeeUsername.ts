...
export class EmployeeUserName extends StringValueObject {
	...
	static async updateUserNameField({
		...
		await EmployeeUserName.ensureEmployeeDoesNotExit({
			repository,
			userName
		})
	...
	static async ensureEmployeeDoesNotExit({
		repository,
		userName
	}: {
		repository: EmployeeRepository
		...
		const emlpoyeeUserName: EmployeePrimitives | null =
			await repository.searchByUserName(userName)
