...
export class CreateEmployeeUseCase {
    ...
    public async execute({ entity, params }: { params: Partial<Omit<EmployeePrimitives, 'id' | 'employeeCode' | 'cedula' | 'nationality'>>, entity: Employee }): Promise<void> {
        ...
        await this.ensureIsStillWorkingUserNameDoesNotExis({ userName: params?.userName, entity })
    ...
    private async ensureIsStillWorkingUserNameDoesNotExis({ entity, userName }: { userName?: Primitives<EmployeeUserName>, entity: Employee }) {
        if (!userName) return
        ...
        if ((await this.employeeRepository.matching(criteria)).length > 0) {
