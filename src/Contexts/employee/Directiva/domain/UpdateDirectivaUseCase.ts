...
export class UpdateDirectivaUseCase {
    ...
    public async execute({ params: { name }, entity }: { entity: Directiva, params: Partial<Omit<DirectivaPrimitives, 'id'>> }): Promise<void> {
        await this.ensureDirectivaDoesNotExist({ name, entity })
    }

    private async ensureDirectivaDoesNotExist({ name, entity }: { name?: Primitives<DepartmentName>, entity: Directiva }): Promise<void> {
        if (!name) return
