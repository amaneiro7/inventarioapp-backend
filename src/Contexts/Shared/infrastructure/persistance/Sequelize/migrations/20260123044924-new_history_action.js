...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		const enumName = 'enum_histories_action'

		try {
			// 2. Añadimos los nuevos valores.
			// IMPORTANTE: 'IF NOT EXISTS' evita errores si la migración se corre dos veces.
			await queryInterface.sequelize.query(`ALTER TYPE "${enumName}" ADD VALUE IF NOT EXISTS 'ASSIGN'`)
			await queryInterface.sequelize.query(`ALTER TYPE "${enumName}" ADD VALUE IF NOT EXISTS 'UNASSIGN'`)
		} catch (error) {
