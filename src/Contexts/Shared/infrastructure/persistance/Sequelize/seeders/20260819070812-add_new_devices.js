...
const { newDeviceToAdd } = require('./newUpdate0826_2/newDeviceData')
...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		try {
			...
			for (const [index, device] of newDeviceToAdd.entries()) {
				const identifier = device.serial || `Registro #${index + 1}`

				// Validar que los IDs existan (que los .find() en newDeviceData.js hayan funcionado)
				if (!device.serial) errors.push(`[${identifier}] El Serial es obligatorio.`)
			...
			if (errors.length > 0) {
				console.error('❌ ERRORES DE VALIDACIÓN DETECTADOS EN newDeviceData.js:')
			...
			for (const device of newDeviceToAdd) {
           ...
           VALUES (:id, :serial, :activo, :statusId, :categoryId, :brandId, :modelId, 
				...
				// await queryInterface.sequelize.query(
				//   `INSERT INTO device_computers (id, category_id, device_id, computer_name, processor_id, memory_ram_capacity, memory_ram, hard_drive_capacity_id, hard_drive_type_id, operating_system_version_id, operating_system_arq_id, mac_address, ip_address, created_at, updated_at)
				//    VALUES (:id, :categoryId, :deviceId, :computerName, :processorId, :memoryRamCapacity, ARRAY[:memoryRam]::NUMERIC[], :hardDriveCapacityId, :hardDriveTypeId, :operatingSystemId, :operatingSystemArqId, :macAddress, :ipAddress, :now, :now)`,
				//   {
				//     replacements: {
				//       id: deviceId,
				//       categoryId: device.categoryId,
				...
				//       memoryRamCapacity: device.memoryRamCapacity,
				//       memoryRam: device.memoriaRamModules,
				//       hardDriveCapacityId: device.hardDriveCapacityId,
				//       hardDriveTypeId: device.hardDriveTypeId,
				//       operatingSystemId: device.operatingSystemId,
				//       operatingSystemArqId: device.operatingSystemArqId,
	...
	async down(queryInterface, Sequelize) {
		const serials = newDeviceToAdd.map(d => d.serial)
		await queryInterface.bulkDelete('devices', {
			serial: { [Sequelize.Op.in]: serials }
		})
		console.log(`Se eliminaron los dispositivos con seriales: ${serials.join(', ')}`)
	}
}
