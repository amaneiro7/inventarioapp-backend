'use strict'

const { newRAMUpdate } = require('./newUpdate0826_2/updateRAM')

const DEVICE_COMPUTER_TABLE = 'device_computers'
const DEVICE_TABLE = 'devices'
/** @type {import('sequelize-cli').Migration} */
module.exports = {
	async up(queryInterface, Sequelize) {
		// Es recomendable envolver todo en una transacción para mayor seguridad
		const transaction = await queryInterface.sequelize.transaction()
		try {
			for (const update of newRAMUpdate) {
				const { serial, memoriaRamModules } = update

				if (serial && memoriaRamModules) {
					const memoryRamCapacity = memoriaRamModules.reduce((acc, val) => acc + val, 0)
					console.log(`Updating device with serial ${serial} to memory RAM capacity ${memoryRamCapacity}`)
					await queryInterface.sequelize.query(
						`UPDATE \"${DEVICE_COMPUTER_TABLE}\" 
          SET \"memory_ram_capacity\" = :memoryRamCapacity, \"memory_ram\" = ARRAY[:memoryRam]::NUMERIC[], \"updated_at\" = :now
          WHERE \"device_id\" IN (
            SELECT \"id\" FROM \"${DEVICE_TABLE}\" WHERE \"serial\" = :serial
          )`,
						{
							replacements: {
								memoryRamCapacity: memoryRamCapacity,
								memoryRam: memoriaRamModules,
								now: new Date(),
								serial: serial
							},
							type: Sequelize.QueryTypes.UPDATE,
							transaction
						}
					)
				}
			}
			await transaction.commit()
		} catch (error) {
			// Si algo falla, deshacemos todo lo anterior
			await transaction.rollback()
			console.error(`Migration failed. Rolling back. Error:`, error)
			throw error // Es importante lanzar el error para que Sequelize sepa que la migración falló
		}
	},

	async down(queryInterface, Sequelize) {
		/**
		 * Add commands to revert seed here.
		 *
		 * Example:
		 * await queryInterface.bulkDelete('People', null, {});
		 */
	}
}
