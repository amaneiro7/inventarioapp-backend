...
const { newOSUpdate } = require('./newUpdate0826_2/updateOS')
	...
	async up(queryInterface, Sequelize) {
		...
		try {
			for (const update of newOSUpdate) {
				const { serial, operatingSystemId } = update

				if (operatingSystemId && serial) {
					console.log(
						`Updating device with serial ${serial} to operating system version ID ${operatingSystemId}`
					)
					await queryInterface.sequelize.query(
						`UPDATE \"${DEVICE_COMPUTER_TABLE}\" 
          SET \"operating_system_version_id\" = :operatingSystemId, \"updated_at\" = :now
          WHERE \"device_id\" IN (
