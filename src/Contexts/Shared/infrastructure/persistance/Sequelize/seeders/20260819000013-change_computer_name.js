...
const { newComputerNameUpdate } = require('./newUpdate0826_2/updateComputerName')
...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		try {
			for (const update of newComputerNameUpdate) {
				const { serial, computerName } = update

				if (computerName && serial) {
					console.log(`Updating device with serial ${serial} to computer name ${computerName}`)
