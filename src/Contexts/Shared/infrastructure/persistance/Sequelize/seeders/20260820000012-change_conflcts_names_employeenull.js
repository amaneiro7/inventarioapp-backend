...
} = require('./newUpdate0826_2/updateDeviceNotConnected')
		...
		try {
			...
			for (const update of changeConflictNameToAlmacen) {
				...
				await queryInterface.sequelize.query(
					...
					{
						replacements: { locationId, employeeId, statusId, oldComputerName, observation, now },
						transaction
					}
				)
				...
				await queryInterface.sequelize.query(
					`UPDATE devices 
						SET status_id = :statusId, 
							...
							updated_at = :now
						WHERE id IN (SELECT device_id FROM device_computers WHERE computer_name = :oldComputerName)`,
					{ replacements: { statusId, employeeId, observation, oldComputerName, now }, transaction }
				)
