...
const { newHDDUpdate } = require('./newUpdate0826_2/updateHDD')
...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		try {
			for (const update of newHDDUpdate) {
				const { serial, hardDriveCapacityId } = update

				if (hardDriveCapacityId && serial) {
					console.log(
						`Updating device with serial ${serial} to hard drive capacity ID ${hardDriveCapacityId}`
					)
					await queryInterface.sequelize.query(
						`UPDATE \"${DEVICE_COMPUTER_TABLE}\" 
          SET \"hard_drive_capacity_id\" = :hardDriveCapacityId, \"updated_at\" = :now
          WHERE \"device_id\" IN (
            SELECT \"id\" FROM \"${DEVICE_TABLE}\" WHERE \"serial\" = :serial
          )`,
						{
							replacements: {
								hardDriveCapacityId: hardDriveCapacityId,
								now: new Date(),
								serial: serial
							},
