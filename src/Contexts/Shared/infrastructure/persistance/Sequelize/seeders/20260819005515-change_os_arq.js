...
const { newOSArqUpdate } = require('./newUpdate0826_2/updateOSArq')
...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		try {
			for (const update of newOSArqUpdate) {
				const { serial, operatingSystemArqId } = update

				if (operatingSystemArqId && serial) {
					console.log(
						`Updating device with serial ${serial} to operating system ARQ ID ${operatingSystemArqId}`
					)
					await queryInterface.sequelize.query(
						`UPDATE \"${DEVICE_COMPUTER_TABLE}\" 
          SET \"operating_system_arq_id\" = :operatingSystemArqId, \"updated_at\" = :now
          WHERE \"device_id\" IN (
