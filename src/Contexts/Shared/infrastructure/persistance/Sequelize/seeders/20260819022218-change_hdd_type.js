...
const { newHDDTypeUpdate } = require('./newUpdate0826_2/updateHDDType')
...
module.exports = {
	async up(queryInterface, Sequelize) {
		...
		try {
			for (const update of newHDDTypeUpdate) {
				const { serial, hardDriveTypeId } = update

				if (hardDriveTypeId && serial) {
					console.log(`Updating device with serial ${serial} to hard drive type ID ${hardDriveTypeId}`)
					await queryInterface.sequelize.query(
						`UPDATE \"${DEVICE_COMPUTER_TABLE}\" 
          SET \"hard_drive_type_id\" = :hardDriveTypeId, \"updated_at\" = :now
          WHERE \"device_id\" IN (
