...
function checkOrphanUnits(units) {
	const orphans = units.filter(u => !u.unidad_id)
	if (orphans.length > 0) {
		console.error(`\n❌ Se encontraron ${orphans.length} unidades de Nivel 6 sin unidad_id:`)
		orphans.forEach(o => {
			console.log(`  - Cedula: "${o.cedula}" | Código: ${o.codigoInterno}`)
		})
	} else {
		console.log('\n✅ Todas los empleados tienen un unidad_id asignado.')
	}
	return orphans
}
