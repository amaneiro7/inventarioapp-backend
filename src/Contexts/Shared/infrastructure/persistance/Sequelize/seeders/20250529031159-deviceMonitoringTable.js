...
module.exports = {
	...
	async down(queryInterface, Sequelize) {
		...
		const computers = await queryInterface.sequelize.query(`SELECT id FROM device_computers;`, {
			type: Sequelize.QueryTypes.SELECT
		})
		...
		if (computerIds.length > 0) {
			...
			console.log(
				`Se eliminaron ${computerIds.length} registros de 'device_monitorings' relacionados con computadoras con IP.`
			)
		} else {
