...
export class Shipment {
	...
	public deliver({
		...
		if (this.status.value !== StatusEnum.IN_TRANSIT) {
			throw new InvalidShipmentStatusTransitionError('Solo se puede entregar un envío que está en tránsito.')
		}
		const deliverDate = deliveryDate ?? new Date()
...
// 		if (params.devices.length === 0 && !params.observation) {
// 			throw new InvalidArgumentError(
// 				'Un envío debe contener al menos un dispositivo o tener una observación obligatoria si no se envía ninguno.'
// 			)
// 		}
