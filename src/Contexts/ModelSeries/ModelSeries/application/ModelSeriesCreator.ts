...
export class ModelSeriesCreator {
  ...
  async run({ name, categoryId, brandId, generic, ...otherParams }: ModelParams): Promise<void> {
    ...
    if (ComputerModels.isComputerCategory({ categoryId })) { // Check if the category is a computer
      ...
      await ComputerMemoryRamType.ensureInputTypeExist({ repository: this.memoryRamTypeRepository, memoryRamTypeId: computerParams.memoryRamTypeId })
      ...
      await ComputerMemoryRamType.ensureInputTypeExist({ repository: this.memoryRamTypeRepository, memoryRamTypeId: laptopParams.memoryRamTypeId })
      ...
      await ModelKeyboardInputType.ensureInputTypeExist({ repository: this.inputTypeRepository, inputTypeId: keyboardParams.inputTypeId })
      ...
      await ModelMouseInputType.ensureInputTypeExist({ repository: this.inputTypeRepository, inputTypeId: mouseParams.inputTypeId })
